...
def get_leaving_task():
    ...
    for i in list(require_chunk.keys()):
        #                        已经完成了        +   正在传输的
        if i not in list(have_finish_chunk.keys()) + getZero() + getOne(): #yzy: 这里增加了一个getOne()，为了进一步筛选包，解决重复下载问题，但是没用
            leave_chunk.append(i)
    return leave_chunk

def get_relex_peer():
    port_list = [str(recv_table[i].from_addr[1]) for i in getZero()]
    ex_self = [i for i in config.peers if i[0] != str(config.identity)]
    ret = [i for i in ex_self if i[2] not in port_list]
    return ret
...
def process_inbound_udp(sock):
    ...
    # receiver
        ...
        if len(ex_received_chunk[chunk_hash]) == CHUNK_DATA_SIZE:
            ...
            logging.debug(f"finished get the chunk {have_finish_chunk.keys()}")
            logging.debug(f"len {len(have_finish_chunk.keys())}")
            ...
            # if time.time()-delay_send_Time>DELAY_TIME:
            if Team == 11:  # 这个是重传的包，不是正常的包
    ...
    # sender
        ...
        if ack_num * MAX_PAYLOAD >= CHUNK_DATA_SIZE:
            ...
            logging.debug(f"send_table {send_table[from_addr].cwnd_window}")
            plt.plot(range(len(send_table[from_addr].cwnd_window)),send_table[from_addr].cwnd_window)
            ...
            send_table[from_addr].limit_time = send_table[from_addr].adapterTimeout() # yzy:这里改了RTT的实现，在send session中多定义了几个变量来实现     


            # test_dupAck
            send_table[from_addr].test_dupAck(ack_num, Team)
...
def getOne(): # yzy
    return [i for i in recv_table.keys() if recv_table[i].tag == 1]
...
class SendSession:
    ...
    def adapterTimeout(self): # yzy
        sampleRTT = time.time() - self.RRT_time
        DevRTT_ = self.DevRTT
        EstimateRTT_ = self.EstimateRTT

        newEstimateRTT  = 0.125*EstimateRTT_+0.875*sampleRTT
        newDevRTT = 0.25*DevRTT_+0.75*abs(sampleRTT-EstimateRTT_)
        newTimeOut =newEstimateRTT+4*newDevRTT

        self.EstimateRTT = newEstimateRTT
        self.DevRTT = newDevRTT
        return newTimeOut
    ...
    def test_dupAck(self, ack_num, team):
        if team == 35:
